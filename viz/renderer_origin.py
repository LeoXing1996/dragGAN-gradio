...
class Renderer:
    ...
    def _render_impl(self, res,
        ...
        # Plot points.
        def plot_point(image, x, y, color, r=3):
            for i in range(-r,r):
                for j in range(-r,r):
                    if y+i >= 0 and y+i < image.size(0) and x+j >= 0 and x+j < image.size(1):
                        image[y+i,x+j,:] = color

        for point in points:
            x = round(point[0] * (res.image.size(1) - 1))
            y = round(point[1] * (res.image.size(0) - 1))
            color = torch.ByteTensor([255,0,0]).to(self._device)
            plot_point(res.image, x, y, color, 3)

        for point in targets:
            x = round(point[0] * (res.image.size(1) - 1))
            y = round(point[1] * (res.image.size(0) - 1))
            color = torch.ByteTensor([0,0,255]).to(self._device)
            plot_point(res.image, x, y, color, 3)
