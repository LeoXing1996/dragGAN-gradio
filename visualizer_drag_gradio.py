...
with gr.Blocks() as app:
    ...
    advanced_usage_cn = """
            ...
            * 在 `w+` 空间上优化可能比 `w` 慢，但通常可以实现更好的结果。
            * 请注意，更改潜在空间将重置图像、点和mask（这与 `重置图像` 按钮具有相同的效果）。
    ...
    def change_language(lang):
        if lang == '中文':
            ...
            form_pretrained_dropdown = gr.update(label="预训练模型")
            form_seed_number = gr.update(label="种子")
            form_lr_number = gr.update(label="步长值")
            form_reset_image = gr.update(value="重置图像")
            enable_add_points = gr.update(value="添加点位")
            undo_points = gr.update(value='重置点位')
            form_start_btn = gr.update(value="开始")
            form_stop_btn = gr.update(value="停止")
            enable_add_mask = gr.update(value='编辑灵活区域')
            form_reset_mask_btn = gr.update(value="重置mask")
            ...
            form_pretrained_dropdown = gr.update(label="Pretrained Model")
            form_seed_number = gr.update(label="Seed")
            form_lr_number = gr.update(label="Step Size")
            form_reset_image = gr.update(value="Reset Image")
            enable_add_points = gr.update(value="Add Points")
            undo_points = gr.update(value='Reset Points')
            form_start_btn = gr.update(value="Start")
            form_stop_btn = gr.update(value="Stop")
            enable_add_mask = gr.update(value='Edit Flexible Area')
            form_reset_mask_btn = gr.update(value="Reset mask")
